...
void ScoreView::paintEvent(QPaintEvent*) {
  ...
  p.fillRect(QRectF(geometry_.pageRect.x, geometry_.pageRect.y,
                    geometry_.pageRect.width, geometry_.pageRect.height),
             Qt::white);
  ...
  for (std::size_t system = 0; system < geometry_.systems; ++system) {
    ...
    for (int line = 0; line < 5; ++line) {
      p.drawLine(QPointF(geometry_.staffLeft,
                         top + line * geometry_.staffSpace),
                 QPointF(geometry_.staffRight,
                         top + line * geometry_.staffSpace));
    }
  }

  const auto drawNote = [&p](double x, double y, QColor color) {
    p.save();
    p.setPen(Qt::NoPen);
    p.setBrush(color);
    p.translate(x, y);
    ...
    p.drawLine(QPointF(x + 5, y), QPointF(x + 5, y - 34));
  };

  const auto diatonicStepsBelowF5 = [](int midi) {
    static constexpr int degreeForPitchClass[12] =
        {0, 0, 1, 1, 2, 3, 3, 4, 4, 5, 5, 6};
    const int octave = midi / 12 - 1;
    const int pitchClass = ((midi % 12) + 12) % 12;
    const int absoluteDegree =
        octave * 7 + degreeForPitchClass[pitchClass];
    const int f5Degree = 5 * 7 + 3;
    return f5Degree - absoluteDegree;
  };

  for (const auto& note : score_.notes()) {
    const double onset = note.onsetBeats.toDouble();
    const double measureWidth =
        (geometry_.staffRight - geometry_.staffLeft) /
        static_cast<double>(geometry_.measuresPerSystem);
    const auto measure = static_cast<std::size_t>(
        onset / static_cast<double>(geometry_.beatsPerMeasure));
    const auto system = measure / geometry_.measuresPerSystem;
    const auto localMeasure = measure % geometry_.measuresPerSystem;
    const double beat =
        onset - static_cast<double>(measure * geometry_.beatsPerMeasure);
    const double x =
        geometry_.staffLeft + localMeasure * measureWidth +
        beat / geometry_.beatsPerMeasure * measureWidth;
    const double y =
        geometry_.firstSystemTop + system * geometry_.systemGap +
        diatonicStepsBelowF5(note.midiPitch) *
            geometry_.staffSpace / 2.0;
    drawNote(x, y, voiceColor(note.voice));
  }

  if (ghost_) {
    drawNote(ghost_->target.snappedPagePoint.x,
             ghost_->target.snappedPagePoint.y,
             ghost_->valid ? voiceColor(ghost_->voice, 105)
                           : QColor(210, 55, 55, 120));
  }
}
