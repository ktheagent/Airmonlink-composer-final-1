...
std::optional<GhostNote> NoteInputController::preview(Point client, const ViewTransform& transform) const {
  const auto target = mapper_.map(client, transform);
  previewVisible_ = target.has_value();
  if (!target) return std::nullopt;
  std::string reason;
  const bool valid = score_.canPlace(target->midiPitch, target->onsetBeats, duration_, voice_, &reason);
  return GhostNote{*target, duration_, voice_, valid, reason, 0.42};
}

std::optional<NoteEvent> NoteInputController::commit(Point client, const ViewTransform& transform) {
  const auto ghost = preview(client, transform);
  if (!ghost || !ghost->valid) return std::nullopt;
  const auto& stored = score_.addNote(ghost->target.midiPitch, ghost->target.onsetBeats, ghost->duration, ghost->voice);
...
bool NoteInputController::redo() {
  ...
  auto note = redoStack_.back();
  redoStack_.pop_back();
  const auto& restored = score_.addNote(note.midiPitch, note.onsetBeats, note.duration, note.voice);
