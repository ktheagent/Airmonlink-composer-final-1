...
class ScoreModel {
 public:
  ...
  double totalBeats() const noexcept;

  const NoteEvent& addNote(int midiPitch, double onset, Duration duration, Voice voice);
  bool removeNote(const std::string& id);
  std::optional<NoteEvent> findNote(const std::string& id) const;
  bool canPlace(int midiPitch, double onset, Duration duration, Voice voice, std::string* reason = nullptr) const;
  std::string deterministicText() const;

 private:
  std::string nextId(int midiPitch, double onset, Duration duration, Voice voice) const;
